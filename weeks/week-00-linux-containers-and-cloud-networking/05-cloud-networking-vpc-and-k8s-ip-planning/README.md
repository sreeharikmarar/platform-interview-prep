...
- **VPC peering constraints**: When two VPCs are peered, their primary subnet ranges cannot overlap (the peering will fail if they do). Critically for GKE, the secondary ranges used for pods and services also participate in the overlap check for VPC peering — GCP peering exchanges subnet routes for both primary and secondary ranges, so they must be non-overlapping with the peer VPC's ranges. This is a common failure point when two teams independently design their GKE clusters and later try to peer the VPCs.

- **VPC peering mechanics — GCP vs AWS**: GCP VPC peering exchanges subnet routes for all ranges (primary and secondary) by default (with the option to disable secondary range export/import), is non-transitive (VPC-A ↔ VPC-B and VPC-B ↔ VPC-C does not give VPC-A → VPC-C connectivity), and is subject to a peering route table limit (~300 routes default, raisable to ~15,000 with quota increase). AWS VPC peering is also non-transitive but supports inter-region peering natively and has a simpler overlap check because there are no secondary ranges — only primary subnet CIDRs participate. A GKE-specific detail: in private GKE clusters, the control plane runs in a Google-managed VPC that is peered to the customer VPC, so the control plane's `/28` master range also consumes peering route table entries. EKS takes a different approach — the control plane injects cross-account ENIs directly into the customer VPC subnets instead of peering.
...
3. **IPAM controller allocation**: The `nodeIpamController` component in `kube-controller-manager` (on self-managed clusters; on managed GKE, this runs inside the Google-managed control plane) watches for Node objects without `spec.podCIDR`. It selects the next available `/24` block from the pod secondary range (tracking used ranges in a persistent bitmap stored in etcd), assigns it to the node by writing `spec.podCIDR` to the Node object, and simultaneously calls the GCE API to add the allocated `/24` as an alias IP range on the node's primary NIC.
