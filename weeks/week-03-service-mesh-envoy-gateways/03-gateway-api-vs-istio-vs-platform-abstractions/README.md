...
- **Policy attachment (GEP-713)**: A Gateway API extension mechanism that allows policy resources to attach to Gateway API resources using a `targetRef` field. GEP-713 defines the attachment pattern and inheritance model (a policy on a Gateway applies to all Routes unless a more-specific policy on a Route overrides it). Concrete policies like `BackendLBPolicy` and `BackendTLSPolicy` exist in v1alpha2. Retry policies are covered by a separate proposal (GEP-1731), not yet widely implemented — most production deployments use VirtualService retry policies instead.
