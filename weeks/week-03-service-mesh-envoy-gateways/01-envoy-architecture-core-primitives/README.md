...
**Mitigation**: Automate certificate rotation using cert-manager or Istio's built-in SPIFFE/X.509 cert rotation (default 24h lifetime, rotated at 80% of lifetime). Monitor certificate expiry with `envoy_ssl_certificate_expiration_timestamp_seconds` metric and alert at 7 days remaining. Use `SAN matching` in `UpstreamTlsContext.combined_validation_context` so upstream TLS fails closed on SAN mismatch rather than accepting any cert. For SDS delays at startup, set a `resource_api_version` and configure `initial_fetch_timeout` on the SDS cluster so Envoy fails fast and shows a clear error rather than hanging.
