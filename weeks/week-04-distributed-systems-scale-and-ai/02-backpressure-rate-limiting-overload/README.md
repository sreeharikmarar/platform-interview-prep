...
Envoy circuit breakers are implemented as atomic counters maintained per-cluster per-thread, aggregated across threads for threshold comparison. The implementation in `source/common/upstream/cluster_manager_impl.cc` (and the `CircuitBreakerImpl` class) tracks four counters for each `RoutingPriority` level (DEFAULT and HIGH). The DEFAULT priority handles ordinary traffic; HIGH priority is set at the route level via explicit `priority: HIGH` in the route action configuration. This separation allows operators to reserve a separate circuit breaker budget for high-priority routes, ensuring they are not starved by normal traffic.
...
`max_requests` tracks total in-flight HTTP requests across all connections. For HTTP/2, a single connection can carry many concurrent streams; for HTTP/1.1, this is effectively equivalent to `max_connections` since each connection carries one request. `max_requests` caps the aggregate count. When a new request would exceed this, it is immediately rejected with 503 and the `UO` response flag is set (there is no dedicated overflow counter for `max_requests`; use the `circuit_breakers.default.rq_open` gauge to confirm).

`max_retries` tracks the number of in-flight retry attempts across all connections. This is the circuit breaker that prevents retry storms at the cluster level. When the in-flight retry count reaches `max_retries`, new retry attempts are rejected (the request fails instead of being retried) and `cluster.<name>.upstream_rq_retry_overflow` increments. This counter is the most important signal for detecting a retry storm in progress. Note that the retry budget (`retry_budget` block) and `max_retries` work together: `retry_budget.budget_percent` dynamically computes the budget as a percentage of currently active requests plus active pending requests, making it adaptive to traffic volume.
...
curl -s http://localhost:9901/stats | grep 'circuit_breakers\|upstream_cx_overflow\|upstream_rq_pending_overflow\|upstream_rq_retry_overflow'
...
```
  ...
  │  retry_budget:                                          │
  │    budget_percent: 10%    (10% of active requests)         │
...
curl -s http://localhost:9901/stats | grep -E \
  'upstream_cx_active|upstream_cx_overflow|upstream_rq_active|upstream_rq_pending_active|upstream_rq_pending_overflow|circuit_breakers.*rq_open'
