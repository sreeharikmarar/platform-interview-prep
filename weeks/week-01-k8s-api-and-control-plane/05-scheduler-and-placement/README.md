...
1. **Pod Watch**: Scheduler's Informer watches all pods. Unscheduled pods (those with `.spec.nodeName == ""`) are enqueued for scheduling. The Pending phase is a consequence of having no node assigned, not a filter criterion.
